...
(async()=>{
  ...
  ],async brand=>({items:[{template:{active:true,recognition:{rows:[{fields:[{type:'STORE_ID',example:brand==='CJ'?'0652':'2982'}]}]}}}]}));
  assert.deepStrictEqual(result.errors,[]);
  assert.strictEqual(result.rows[0].storeCode,'CJ539');
