...
object PosEvidenceFusion {
    ...
    fun apply(
        ...
        byPos.forEach { (pos, posEvidence) ->
            val templateGroup = chooseTemplateGroup(posEvidence) ?: return@forEach
            val template = templateGroup.first().template
            val passCount = rawTexts.count { it.isNotBlank() }
            val minimumSupport = if (passCount >= 3) 2 else 1

            val customer = resolveField(templateGroup, "CUSTOMER_VALUE", minimumSupport) { value ->
                value.isNotBlank() && value.all(Char::isDigit)
            }
            val date = resolveField(templateGroup, "BILL_DATE", minimumSupport, ::isValidDate)
            val time = resolveField(templateGroup, "BILL_TIME", minimumSupport, ::isValidTime)
            val store = resolveField(templateGroup, "STORE_ID", minimumSupport) { it.isNotBlank() }

            val core = template.validation.requiredCore
            if (core.customerValue && customer == null) return@forEach
            if (core.date && date == null) return@forEach
            if (core.time && time == null) return@forEach
            ...
            val supports = listOfNotNull(customer, date, time).map { it.support }
            val weakestSupport = supports.minOrNull() ?: 1
            val warning = if (weakestSupport <= 1 && passCount >= 3) {
                "ข้อมูลบางช่องพบชัดเพียงรอบเดียว กรุณาตรวจเทียบกับภาพก่อนส่ง"
            } else ""
    ...
    internal fun fuseTextPasses(
        ...
        return evidence.groupBy { it.pos }.mapValues { (_, all) ->
            val group = chooseTemplateGroup(all).orEmpty()
            val minimumSupport = if (rawTexts.count { it.isNotBlank() } >= 3) 2 else 1
            buildMap {
                put("POS_NUMBER", all.first().pos.toString())
                resolveField(group, "CUSTOMER_VALUE", minimumSupport) { it.all(Char::isDigit) }?.let {
                    put("CUSTOMER_VALUE", it.value)
                }
                resolveField(group, "BILL_DATE", minimumSupport, ::isValidDate)?.let {
                    put("BILL_DATE", it.value)
                }
                resolveField(group, "BILL_TIME", minimumSupport, ::isValidTime)?.let {
                    put("BILL_TIME", it.value)
                }
                resolveField(group, "STORE_ID", minimumSupport) { it.isNotBlank() }?.let {
                    put("STORE_ID", it.value)
                }
            }
        }.filterValues { fields ->
            val core = template.validation.requiredCore
            (!core.customerValue || !fields["CUSTOMER_VALUE"].isNullOrBlank()) &&
                (!core.date || !fields["BILL_DATE"].isNullOrBlank()) &&
                (!core.time || !fields["BILL_TIME"].isNullOrBlank())
        }
    }
    ...
    private fun chooseTemplateGroup(evidence: List<Evidence>): List<Evidence>? = evidence
        .groupBy { it.template.templateId }
        .values
        .maxWithOrNull(
            compareBy<List<Evidence>> { group -> group.map { it.passIndex }.distinct().size }
                .thenBy { group -> group.maxOfOrNull { it.depth } ?: 0 }
                .thenBy { group -> group.sumOf { it.score } }
        )
